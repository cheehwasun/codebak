...
def lzma_func():
    ...
    # 异常
    try: pass
    except lzma.LZMAError: pass  # 在压缩或解压或初始化时发生错误
...
def zip_func():
    ...
    # 异常
    try: pass
    except zipfile.BadZipFile: pass  # zip文件引发的错误
    except zipfile.LargeZipFile: pass  #  需要ZIP64功能, 但未启用
...
def tarfile_func():
    ...
    # 异常
    try: pass
    except tarfile.TarError: pass  # tarfile异常的基类
    except tarfile.ReadError: pass  # tar文件打开时引发的异常
    except tarfile.CompressionError: pass  # 无法解码
    except tarfile.StreamError: pass # 数据流异常
    except tarfile.ExtractError: pass  # TarFile.extract()(解压)引发的异常
    except tarfile.HeaderError: pass  # TarInfo.frombuf()(缓冲区无效)引发的异常
